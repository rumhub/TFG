...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\For{época $p\in\{0, ..., P-1\}$}
			\State Trabajador 0 desordena vector de datos D.
			\State Sincronizar trabajadores
				\For{cada mini batch $m =0, ..., M-1$}
					\State Inicializar $gradientes^t$ a 0.
					\State $(x_i, y_i)^t\ \gets $ Reparto de datos del batch m al trabajador t
					\State Realizar propagación hacia delante de ${x_i}^t$ y obtener 
					\State predicción ${\hat{y}_i}^t$.
					\State Evaluar ${\hat{y}_i}^t$ con la función de pérdida y calcular el error ${e_i}^t$.
