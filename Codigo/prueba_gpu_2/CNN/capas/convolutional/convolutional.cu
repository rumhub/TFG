...
{
    float *X = (float*)malloc(H*W * sizeof(float));
...
void Convolutional::backPropagation_vectores_externos(float *input, float *output, float *a, float *grad_w, float *grad_bias)
{
    ...
    // -----------------------------
    // cout << "input\n";
    // mostrar_tensor2(H, W, input);

    // int M = kernel_fils*kernel_cols*C* H_out*W_out;
    // // float *h_out = (float *)malloc(M * sizeof(float));
    // // cudaMemcpy(h_out, d_input_back_unroll, M * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    // //         h_out[i] = 1.0;
    // // }
    // // cudaMemcpy(d_input_back_unroll, h_out, M * sizeof(float), cudaMemcpyHostToDevice);
    // // cout << kernel_fils*kernel_cols*C << endl;
    // cout << "d_input_back_unroll\n";
    // mostrar_tensor2(kernel_fils*kernel_cols*C, H_out*W_out, d_input_back_unroll);
    ...
    // mostrar_tensor2(H_out*W_out, kernel_fils*kernel_cols*C, d_input_back_unroll_T);
    // cout << "output\n";
    // mostrar_tensor2(n_kernels, H_out*W_out, output);
    // // -----------------------
