...
void CNN::train(int epocas, int mini_batch)
{
    ...
    float *pool_out = (float*)malloc(128*tam_ini * sizeof(float));
    ...
    for(int ep=0; ep<epocas; ++ep)
    {
        ...
        for(int i=0; i<n_batches; ++i)
        {
            ...
            for(int img=0; img<tam_batches[i]; ++img)
            {
                ...
                // ------------------------------------
                cudaMemcpy(pool_out, d_img_conv_out, this->convs[i_c].get_n_kernels()*this->convs[i_c].get_H_out()*this->convs[i_c].get_W_out() * sizeof(float), cudaMemcpyDeviceToHost);

                for(int i_=0; i_<this->convs[i_c].get_n_kernels()*this->convs[i_c].get_H_out()*this->convs[i_c].get_W_out(); i_++)
                    if(pool_out[i_] != 0.0)
                        pool_out[i_] = 1.0;

                cudaMemcpy(d_img_conv_out, pool_out, this->convs[i_c].get_n_kernels()*this->convs[i_c].get_H_out()*this->convs[i_c].get_W_out() * sizeof(float), cudaMemcpyHostToDevice);
                // ------------------------------------


                cout << "Conv_a_" << i_c << "\n";
                mostrar_tensor(this->convs[i_c].get_n_kernels(), this->convs[i_c].get_H_out(), this->convs[i_c].get_W_out(), pool_out, d_img_conv_out);
