...
void mostrar_tensor2(int H, int W, float *d_X)
{
		float *X = (float *)malloc(64*64*64 * sizeof(float));
    cudaMemcpy(X, d_X, H*W * sizeof(float), cudaMemcpyDeviceToHost);
...
void Convolutional::backPropagation_vectores_externos(float *input, float *output, float *a, float *grad_w, float *grad_bias)
{
    ...
    unroll_3dim_gpu<<<grid, block>>>(n_kernels, H_out_pad, W_out_pad, kernel_fils, d_output_pad, d_output_unroll);		// "Desenrrollar" volumen de salida
		...
		// -----------------------------
		/*
		float *h_out = (float *)malloc(cols_output_unroll * fils_output_unroll * sizeof(float));
		cudaMemcpy(h_out, d_output_unroll, cols_output_unroll * fils_output_unroll * sizeof(float), cudaMemcpyDeviceToHost);

		for(int i=0; i<cols_output_unroll * fils_output_unroll; i++)
		{
			if(h_out[i] != 0.0)
				h_out[i] = 1.0;
		}
		cudaMemcpy(d_output_unroll, h_out, cols_output_unroll * fils_output_unroll * sizeof(float), cudaMemcpyHostToDevice);
		cout << kernel_fils*kernel_cols*C << endl;
		cout << "d_input_back_unroll\n";
		mostrar_tensor2(H*W, kernel_fils*kernel_cols*C, d_output_unroll);
		*/
		// -----------------------------


    // Transpuesta del volumen de salida desenrrollado
    grid.y = (cols_output_unroll + BLOCK_SIZE -1) / BLOCK_SIZE;
    grid.x = (fils_output_unroll + BLOCK_SIZE -1) / BLOCK_SIZE;
    matrizTranspuesta_conv<<<grid, block>>>(d_output_unroll, d_output_unroll_T, cols_output_unroll, fils_output_unroll);

		// ------------------------
		//cout << "d_input_back_unroll_T\n";
		//mostrar_tensor2(kernel_fils*kernel_cols*C, H*W, d_output_unroll_T);
		// -----------------------
